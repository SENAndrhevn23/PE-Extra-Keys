	...
	public function spawnNotes()
	{
		...
		for (i in 0...dataArray.length)
		{
			if(Note.globalRgbShaders[i] == null)
			{
				...
				Note.globalRgbShaders[i] = newRGB;

				var arr:Array<FlxColor> = !onPixel ? ClientPrefs.data.arrowRGBExtra[i] : ClientPrefs.data.arrowRGBPixelExtra[i];
				newRGB.r = arr[0];
				newRGB.g = arr[1];
				newRGB.b = arr[2];
			}
		...
		bigNote.frames = Paths.getSparrowAtlas(skin);
