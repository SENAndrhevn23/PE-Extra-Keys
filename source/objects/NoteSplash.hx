			...
			if (inEditor || (note == null || note.noteSplashData.useRGBShader) && (PlayState.SONG == null || !PlayState.SONG.disableNoteRGB))
			{
				...
				if ((note == null || !note.noteSplashData.useGlobalShader) || inEditor)
				{
					...
					if (colors != null)
					{
						...
						var arr:Array<FlxColor> = ClientPrefs.data.arrowRGBExtra[noteData % Note.colArray.length];
						if (PlayState.isPixelStage) arr = ClientPrefs.data.arrowRGBPixelExtra[noteData % Note.colArray.length];
