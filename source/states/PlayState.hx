...
class PlayState extends MusicBeatState
{
	...
	private function generateSong():Void
	{
		...
		for (section in sectionsData)
		{
			...
			for (i in 0...section.sectionNotes.length)
			{
				...
				var noteColumn: Int = Std.int(songNotes[1] % (Main.mania+1));
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if(char != null && (note == null || !note.noMissAnimation) && char.hasMissAnimations)
		{
			...
			var animToPlay:String = singAnimations[Note.gfxHud[Main.mania][Std.int(Math.abs(direction))]] + 'miss' + postfix;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		{
			var char:Character = dad;
				var animToPlay:String = singAnimations[Note.gfxHud[Main.mania][Std.int(Math.abs(note.noteData))]] + note.animSuffix;
