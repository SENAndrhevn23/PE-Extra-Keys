...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function create()
	{
		...
		{
			if(PlayState.SONG.mania == null || PlayState.SONG.mania < 0 || PlayState.SONG.mania > 8)
	...
	function addSongTab()
	{
		...
		maniaStepper = new PsychUINumericStepper(objX + 180, objY, 1, 3, 0, 8, 0);
	...
	function getMania():Int
	{
		if(PlayState.SONG != null && PlayState.SONG.mania != null)
			return Std.int(FlxMath.bound(PlayState.SONG.mania, 0, 8));
		return 3;
	}

	function setMania(value:Int)
	{
		value = Std.int(FlxMath.bound(value, 0, 8));
